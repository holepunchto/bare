...
#### `const thread = new Thread(filename, source[, options])`

#### `const thread = new Thread(filename[, options], callback)`

Start a new thread that will run `source`, which is a string or a `Buffer`. If `callback` is given instead, its function body will be used as the source and invoked on the new thread with `Thread.self.data` passed as an argument.
