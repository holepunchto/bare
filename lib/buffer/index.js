...
module.exports = class Buffer extends Uint8Array {
  ...
  copy (target, targetStart = 0, sourceStart = 0, sourceEnd = this.byteLength) {
    if (!(target instanceof Uint8Array)) throw new TypeError('target must be a buffer')
    subarrayMaybe(target, targetStart).set(subarrayMaybe(this, sourceStart, sourceEnd))
    return Math.min(target.byteLength - targetStart, sourceEnd - sourceStart)
  }
...
function subarrayMaybe (buffer, start = 0, end = buffer.byteLength) {
