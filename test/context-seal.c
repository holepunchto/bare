...
// readable; nothing may be published or withdrawn after.
...
main(int argc, char *argv[]) {
  ...
  // Neither half of the registry may be written once the process is sealed.
  e = bare_context_set(bare, "bare.test.late.v1", &bare_test__value, NULL);
  assert(e < 0);

  e = bare_context_delete(bare, "bare.test.value.v1");
  assert(e < 0);
