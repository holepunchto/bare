...
// An addon may be constructed long after the script that loaded it returned,
// and from a loop that the embedder drives itself rather than through
// `bare_run()`. Context is resolved against the process running on the thread,
// which the addon must be able to find however it was reached.
...
static const char *code =
  "const { Addon } = Bare\n"
  "setTimeout(() => new Addon(new URL('builtin:context-addon-deferred')), 0)\n";
...
main(int argc, char *argv[]) {
  ...
  // The timer runs on an iteration of the loop that the embedder drives, so the
  // addon initialises with the runtime entered through nothing of its own.
